...
作为类比，完全节点就像一个处在陌生城市的游客，带了含有所有街道所有地址的详细地图。而SPV节点就像另一个同在陌生城市的游客，只知道一条主干道，通过随机询问陌生人来获知路线规划。虽然两个游客都能通过实地参观验证街道是否存在，但没有地图的游客不知道每个小巷中有些什么，也不知道附近还有什么其他街道。站在教堂街23号前，没有地图的游客无法知道这个城市是否还有其他“教堂街23号”的地址，也不知道这个地方是不是就是自己要找的一个。对于没有地图的游客，最好的办法就是询问足够多的人，并希望不会有人试图抢劫他。

简单支付验证通过引用交易在区块链中的*深度（depth）*而不是它们的*高度（height）*来验证交易。而完全区块链节点则创建一个完整的验证过的区块链，这条链由区块和交易组成，并按时间倒序一直追溯到创世区块。而一个SPV节点会验证所有区块的链（但不是所有的交易），并且把链和有关感兴趣的交易进行关联。

比如，当检查区块300,000中的某个交易时，一个完全节点将从区块300,000开始一直往前追溯到创世区块，将所有300,000个区块连接起来，并建立起一个完整的UTXO数据库，通过确认UTXO未被花费，来确立交易的有效性。SPV节点无法确定UTXO是否未被花费。相反，SPV节点则利用*默克尔路径*（参看第164页《默克尔树》）建立起交易和包含这笔交易的区块间的关联关系。接着，SPV节点一直等到序号从300,001到300,006的六个区块堆叠在该交易所在的区块之上，并通过确定交易的深度是在第300,006区块到第300,001区块之下来验证交易的有效性。事实上，网络上的其他节点接受了区块300,000，并在其上创建了额外的6个区块，根据代理协议，可以证明交易不是一个双重支付交易。
