...
交易是比特币系统中最重要的部分，其他的设计，比如确保交易顺利创建，在网络中传播，验证，最后加入到全局交易账本（区块链）等技术都是为交易服务的。交易是一个数据结构，用于对比特币系统中不同参与者间的价值传递进行编码。每个交易都是比特币区块链中的一条公开记录，而区块链则是一个全局的复式簿记账本。

在本章中，我们将研究交易的各种形态，比如它们包含什么内容，如何创建，如何验证，又是如何成为永久交易记录的一部分的。
